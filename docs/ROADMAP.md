...
| ccru license | Phase 1 (ask now) | @lumpenspace's ccru library has **no published license** (all-rights-reserved by default). Fine informally for personal use; ask him to add MIT (or give written permission) — required before Pylot is ever distributed. Record the outcome in `Themes/cyber_theme/vendor/ccru/UPSTREAM.md` |
| Spoiler policy | Phase 1 | The owner is both builder and learner: reference solutions and answer keys are spoilers. Policy: owner reviews app code and content *structure* freely, but does not read `reference_solution` blocks or generated answer keys for chapters she hasn't passed — the validator is what vouches for them |
